...
- `src/components/game/LeaveButton.tsx`, `SettingsButton.tsx` + `SettingsMenu.tsx` — top-bar icons; the settings popover holds a "3D dice" `Switch` (BETA-badged, persisted server-side via `updateGameModeAction`) and a Sound on/off `Switch` (persisted to `localStorage`, `src/lib/sound.ts`'s `isMuted`/`setMuted`) above a separator, then Legend/About. Both the mode switch and Next-player/Players/Hand nav disable for the whole `rollState !== "idle"` window (rolling through the result banner) — a roll (especially an in-flight 3D physics one) can't be cleanly cancelled mid-flight, so the escape hatch is blocked instead.
...
1. Deploy to Vercel + swap local Postgres for Neon — the goal of phase06, alongside general polish, ahead of a public release.
