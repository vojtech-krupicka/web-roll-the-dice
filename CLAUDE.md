...
- **Icons:** `lucide-react`.
- **Database:** Postgres (Neon, serverless-friendly) via **Drizzle ORM** — not started yet (planned from phase 02+). Local dev DB: Postgres 17 in Docker, reachable at `localhost:5432` (works from Windows directly; Docker Desktop bridges WSL2↔Windows `localhost`, so this isn't WSL-dependent).
...
- **Two roll modes (planned):**
  - **2D:** flat sprites, result from `Math.random()`.
  - **3D:** `@react-three/fiber` + `@react-three/rapier` (or `@3d-dice/dice-box`) physics sim. Since this isn't multiplayer, there's no server-authoritative requirement — the result can be computed client-side however is convenient, animated, shown, then persisted to roll history.

## Current state (as of phase01)

Just a local, single-die interaction — no DB, no games/players, no routing beyond `/`:

- `src/app/page.tsx` — owns the roll state machine: `idle → rolling → result → idle`.
- `src/components/dice/DieFace.tsx` — SVG pip-layout renderer for a d6 face (1–6); reusable foundation for future dice types.
- `src/components/dice/DropArea.tsx` — top ~2/3 of viewport, centers the die.
- `src/components/dice/RollButton.tsx` — the big roll CTA.
- `src/components/dice/ResultPopup.tsx` — full-screen dismiss layer + result bubble.
- `src/lib/dice.ts` — `rollD6()` and roll-animation timing constants.
...
4. Support for other dice types (d4/d8/d10/d12/d20/d100) and multiple dice per roll.
5. 3D physics roll mode.
6. Deploy to Vercel + swap local Postgres for Neon.
